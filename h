...
OOPPTTIIOONNSS
       ...
       <refspec>...
           ...
           that <<ssrrcc>> normally updates without any <<rreeffssppeecc>> on the command line. Otherwise, missing ::<<ddsstt>> means to update the same ref
           as the <<ssrrcc>>.
           ...
           fast-forwarded is not a commit, but a tag object which happens to point to a new commit which is a fast-forward of the commit
           the last tag (or commit) it’s replacing. Replacing a tag with an entirely different tag is also allowed, if it points to the
           same commit, as well as pushing a peeled tag, i.e. pushing the commit that existing tag object points to, or a new tag object
           ...
           leading ++ in the refspec (or ----ffoorrccee), except when forbidden by configuration or hooks. See rreecceeiivvee..ddeennyyDDeelleetteess in ggiitt--
       ...
       --mirror
           ...
           end, locally updated refs will be force updated on the remote end, and deleted refs will be removed from the remote end. This
           is the default if the configuration option rreemmoottee..<<rreemmoottee>>..mmiirrrroorr is set.
       ...
       -o <option>, --push-option=<option>
           ...
           string must not contain a NUL or LF character. When multiple ----ppuusshh--ooppttiioonn==<<ooppttiioonn>> are given, they are all sent to the other
           side in the order listed on the command line. When no ----ppuusshh--ooppttiioonn==<<ooppttiioonn>> is given from the command line, the values of
       ...
       --[no-]force-with-lease, --force-with-lease=<refname>, --force-with-lease=<refname>:<expect>
           ...
           original history while you are rebasing, the tip of the branch at the remote may advance with her commit, and blindly pushing
           with ----ffoorrccee will lose her work.
       ...
       --no-recurse-submodules, --recurse-submodules=check|on-demand|only|no
           ...
           _o_n_-_d_e_m_a_n_d is used all submodules that changed in the revisions to be pushed will be pushed. If on-demand was not able to push
           all necessary revisions it will also be aborted and exit with non-zero status. If _o_n_l_y is used all submodules will be
...
GGIITT UURRLLSS
       ...
       These two syntaxes are mostly equivalent, except when cloning, when the former implies --local option. See ggiitt--cclloonnee(1) for
       details.
       ...
       invoked. See ggiittrreemmoottee--hheellppeerrss(7) for details.
       ...
       a URL like "git://example.org/path/to/repo.git" will be rewritten to "ssh://example.org/path/to/repo.git" for pushes, but pulls
...
RREEMMOOTTEESS
   ...
   NNaammeedd rreemmoottee iinn ccoonnffiigguurraattiioonn ffiillee
       You can choose to provide the name of a remote which you had previously configured using ggiitt--rreemmoottee(1), ggiitt--ccoonnffiigg(1) or even by
       a manual edit to the $$GGIITT__DDIIRR//ccoonnffiigg file. The URL of this remote will be used to access the repository. The refspec of this
       remote will be used by default when you do not provide a refspec on the command line. The entry in the config file would appear
       like this:
   ...
   NNaammeedd ffiillee iinn $$GGIITT__DDIIRR//rreemmootteess
       ...
       PPuusshh:: lines are used by _g_i_t _p_u_s_h and PPuullll:: lines are used by _g_i_t _p_u_l_l and _g_i_t _f_e_t_c_h. Multiple PPuusshh:: and PPuullll:: lines may be
   ...
   NNaammeedd ffiillee iinn $$GGIITT__DDIIRR//bbrraanncchheess
       You can choose to provide the name of a file in $$GGIITT__DDIIRR//bbrraanncchheess. The URL in this file will be used to access the repository.
       ...
       Depending on the operation, git will use one of the following refspecs, if you don’t provide one on the command line. <<bbrraanncchh>> is
       the name of this file in $$GGIITT__DDIIRR//bbrraanncchheess and <<hheeaadd>> defaults to mmaasstteerr.
...
OOUUTTPPUUTT
       ...
       The status of the push is output in tabular form, with each line representing the status of a single ref. Each line is of the
       form:
       ...
       summary
           ...
           argument to ggiitt lloogg (this is <<oolldd>>....<<nneeww>> in most cases, and <<oolldd>>......<<nneeww>> for forced non-fast-forward updates).
           ...
           remote rejected
               The remote end refused the update. Usually caused by a hook on the remote side, or because the remote repository has one
               of the following safety options in effect: rreecceeiivvee..ddeennyyCCuurrrreennttBBrraanncchh (for pushes to the checked out branch),
               rreecceeiivvee..ddeennyyNNoonnFFaassttFFoorrwwaarrddss (for forced non-fast-forward updates), rreecceeiivvee..ddeennyyDDeelleetteess or rreecceeiivvee..ddeennyyDDeelleetteeCCuurrrreenntt. See
               ggiitt--ccoonnffiigg(1).

           remote failure
               The remote end did not report the successful update of the ref, perhaps because of a temporary error on the remote side,
       ...
       reason
           A human-readable explanation. In the case of successfully pushed refs, no explanation is needed. For a failed ref, the reason
           for failure is described.
...
NNOOTTEE AABBOOUUTT FFAASSTT--FFOORRWWAARRDDSS
       ...
       X, and you built a history leading to commit B while the other person built a history leading to commit A. The history looks like
       this:
       ...
       But if you try to push, you will attempt to update the branch (that now points at A) with commit B. This does _n_o_t fast-forward.
       ...
       If you do not want to lose your work (history from X to B) or the work by the other person (history from X to A), you would need
       to first fetch the history from the repository, create a history that contains changes done by both parties, and push the result
       back.
       ...
       this section), replace it with "git commit --amend" to produce commit B, and you try to push it out, because forgot that you have
       pushed A out already. In such a case, and only if you are certain that nobody in the meantime fetched your earlier commit A (and
       started building on top of it), you can run "git push --force" to overwrite it. In other words, "git push --force" is a method
       reserved for a case where you do mean to lose history.

EEXXAAMMPPLLEESS
       ggiitt ppuusshh
           Works like ggiitt ppuusshh <<rreemmoottee>>, where <remote> is the current branch’s remote (or oorriiggiinn, if no remote is configured for the
           current branch).

       ggiitt ppuusshh oorriiggiinn
           Without additional configuration, pushes the current branch to the configured upstream (rreemmoottee..oorriiggiinn..mmeerrggee configuration
           variable) if it has the same name as the current branch, and errors out without pushing otherwise.

           The default behavior of this command when no <refspec> is given can be configured by setting the ppuusshh option of the remote,
           or the ppuusshh..ddeeffaauulltt configuration variable.

           For example, to default to pushing only the current branch to oorriiggiinn use ggiitt ccoonnffiigg rreemmoottee..oorriiggiinn..ppuusshh HHEEAADD. Any valid
           <refspec> (like the ones in the examples below) can be configured as the default for ggiitt ppuusshh oorriiggiinn.
       ...
       ggiitt ppuusshh oorriiggiinn mmaasstteerr
           Find a ref that matches mmaasstteerr in the source repository (most likely, it would find rreeffss//hheeaaddss//mmaasstteerr), and update the same
           ref (e.g.  rreeffss//hheeaaddss//mmaasstteerr) in oorriiggiinn repository with it. If mmaasstteerr did not exist remotely, it would be created.
       ...
       ggiitt ppuusshh mmootthheerrsshhiipp mmaasstteerr::ssaatteelllliittee//mmaasstteerr ddeevv::ssaatteelllliittee//ddeevv
           Use the source ref that matches mmaasstteerr (e.g.  rreeffss//hheeaaddss//mmaasstteerr) to update the ref that matches ssaatteelllliittee//mmaasstteerr (most
           probably rreeffss//rreemmootteess//ssaatteelllliittee//mmaasstteerr) in the mmootthheerrsshhiipp repository; do the same for ddeevv and ssaatteelllliittee//ddeevv.

           See the section describing <<rreeffssppeecc>>......  above for a discussion of the matching semantics.

           This is to emulate ggiitt ffeettcchh run on the mmootthheerrsshhiipp using ggiitt ppuusshh that is run in the opposite direction in order to integrate
           the work done on ssaatteelllliittee, and is often necessary when you can only make connection in one way (i.e. satellite can ssh into
           mothership but mothership cannot initiate connection to satellite because the latter is behind a firewall or does not run
           sshd).

           After running this ggiitt ppuusshh on the ssaatteelllliittee machine, you would ssh into the mmootthheerrsshhiipp and run ggiitt mmeerrggee there to complete
           the emulation of ggiitt ppuullll that were run on mmootthheerrsshhiipp to pull changes made on ssaatteelllliittee.

       ggiitt ppuusshh oorriiggiinn HHEEAADD::mmaasstteerr
           Push the current branch to the remote ref matching mmaasstteerr in the oorriiggiinn repository. This form is convenient to push the
       ...
       ggiitt ppuusshh oorriiggiinn mmaasstteerr::rreeffss//hheeaaddss//eexxppeerriimmeennttaall
           Create the branch eexxppeerriimmeennttaall in the oorriiggiinn repository by copying the current mmaasstteerr branch. This form is only needed to
           create a new branch or tag in the remote repository when the local name and the remote name are different; otherwise, the ref
       ...
       ggiitt ppuusshh oorriiggiinn ++ddeevv::mmaasstteerr
           Update the origin repository’s master branch with the dev branch, allowing non-fast-forward updates.  TThhiiss ccaann lleeaavvee
           uunnrreeffeerreenncceedd ccoommmmiittss ddaanngglliinngg iinn tthhee oorriiggiinn rreeppoossiittoorryy..  Consider the following situation, where a fast-forward is not
           possible:
           ...
           would be removed by a ggiitt ggcc command on the origin repository.
...
SSEECCUURRIITTYY
       ...
       intended to be shared. If you have private data that you need to protect from a malicious peer, your best option is to store it
       in another repository. This applies to both clients and servers. In particular, namespaces on a server are not effective for read
       access control; you should only grant read access to a namespace to clients that you would trust with read access to the entire
       repository.

       The known attack vectors are as follows:

        1. The victim sends "have" lines advertising the IDs of objects it has that are not explicitly intended to be shared but can be
           used to optimize the transfer if the peer also has them. The attacker chooses an object ID X to steal and sends a ref to X,
           but isn’t required to send the content of X because the victim already has it. Now the victim believes that the attacker has
           X, and it sends the content of X back to the attacker later. (This attack is most straightforward for a client to perform on
           a server, by creating a ref to X in the namespace the client has access to and then fetching it. The most likely way for a
           server to perform it on a client is to "merge" X into a public branch and hope that the user does additional work on this
        ...
        2. As in #1, the attacker chooses an object ID X to steal. The victim sends an object Y that the attacker already has, and the
           attacker falsely claims to have X and not Y, so the victim sends Y as a delta against X. The delta reveals regions of X that
           are similar to Y to the attacker.

GGIITT
       Part of the ggiitt(1) suite

Git 2.25.1                                                     09/09/2021                                                    GIT-PUSH(1)
